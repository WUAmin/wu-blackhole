...
from helper import ChecksumType, EncryptionType, chacha20poly1305_encrypt_data, compress_bytes_to_string_b64zlib, \
    get_checksum_sha256_file, get_checksum_sha256_folder
from wbh_item import QueueState, WBHChunk, WBHItem
...
class WBHQueue:
    ...
    def is_item_exist_list(self, item: WBHItem, items: list) -> bool:
    ...
    def save(self):
        ...
        settings.logger_core.debug("Saving queue to `{}`".format(self.queue_file))
        try:
            with open(self.queue_file, 'w') as f:
                json.dump([o.to_dict() for o in self.items], f, sort_keys=False)
        except Exception as e:
            settings.logger_core.error("  ERROR: Can not save queue to `{}`:\n {}".format(self.queue_file, str(e)))
        settings.logger_core.debug("  Queue saved with {} items".format(len(self.items)))


    def load(self):
        from settings import settings
        """ return true if loaded queue successfully from disk"""
        settings.logger_core.debug("Loading queue from `{}`".format(self.queue_file))
        try:
            ...
            settings.logger_core.error("  ERROR: Can not load queue from `{}`:\n {}".format(self.queue_file, str(e)))
        settings.logger_core.debug("  Queue loaded with {} items".format(len(self.items)))


    @staticmethod
    def backup_database(blackhole):
        from settings import settings
        settings.logger_core.debug("Sending database backup to blackhole...")
        try:
            db_root_path, db_filename = os.path.split(settings.Database.get_db_filepath())
            # Create a new WBHItem for database backup
            db_wbhi: WBHItem = WBHItem(size=os.stat(settings.Database.get_db_filepath()).st_size,
                                       full_path=settings.Database.get_db_filepath(),
                                       root_path=db_root_path,
                                       filename=db_filename,
                                       is_dir=False,
                                       state=QueueState.UPLOADING,
                                       modified_at=os.path.getmtime(settings.Database.get_db_filepath()),
                                       created_at=os.path.getctime(settings.Database.get_db_filepath()),
                                       checksum=get_checksum_sha256_file(settings.Database.get_db_filepath()),
                                       checksum_type=ChecksumType.SHA256)
            # Send Database backup to blackhole
            if settings.TelegramBot.send_file(item_wbhi=db_wbhi,
                                              blackhole=blackhole,
                                              chunk_size=settings.config['general']['chunk_size'],
                                              temp_dir=settings.config['general']['tempdir'],
                                              encryption_type=EncryptionType.ChaCha20Poly1305,
                                              encryption_secret=settings.config['general']['backup_pass']):
                ...
                encrypted_data, key, nonce = chacha20poly1305_encrypt_data(data=raw_db_backup_data,
                                                                           secret=settings.config['general'][
                                                                               'backup_pass'].encode())
                ...
                for sp in str_parts:
                    ...
                    settings.TelegramBot.send_msg(chat_id=blackhole.telegram_id,
                                                  text="{}\nb64zlib\n```{}```\n#WBHBackup"
                                                  .format(datetime.today().strftime('%Y-%m-%d %H:%M:%S'), sp))
                return str_parts
            else:
                settings.logger_core.error("ERROR: Could send encrypted database backup  to BlackHole!!!")
        except Exception as e:
    ...
    def process_queue_list(self, telegram_id: str, items: list, parent: WBHItem = None):
            ...
            if item.is_dir:
                ...
                try:
                    if item.state == QueueState.INQUEUE:
                        ...
                        # get item checksum
                        item.checksum = get_checksum_sha256_folder(item.full_path, logger=settings.logger_core)
                        item.checksum_type = ChecksumType.SHA256
                        # Add to Database and Update db_id on queue item
                        item.db_id = settings.Database.add_item(item_wbhi=item, blackhole_id=self.blackhole.id,
                    ...
                    if item.state == QueueState.DONE:
                        ...
                        if item.children:
                            if self.process_queue_list(telegram_id, item.children, item):
                                ...
                                settings.logger_core.debug("`{}` removed from disk.".format(item.filename))
                    ...
                    if item.state == QueueState.DELETED and parent is None:
                        ...
                        self.remove(item)
                        settings.logger_core.debug("`{}` removed from queue.".format(item.filename))
                        settings.logger_core.info("`{}` has been sent to blackhole.".format(item.filename))
                        # Save Queue to disk
                        self.save()
                except Exception as e:
                    settings.logger_core.error("ERROR: Could add item `{}` to BlackHole: {}"
                ...
                if item.state == QueueState.INQUEUE:
                    try:
                        ...
                        item.state = QueueState.UPLOADING
                        # get item checksum
                        item.checksum = get_checksum_sha256_file(item.full_path, logger=settings.logger_core)
                        item.checksum_type = ChecksumType.SHA256
                        ...
                        settings.logger_core.error("ERROR: Could add item `{}` to Database: {}"
                                                   .format(item.filename, str(e)))
                if item.state == QueueState.UPLOADING:
                    try:
                        everything_is_done = False
                        # Send File to blackhole
                        if settings.TelegramBot.send_file(item_wbhi=item, blackhole=self.blackhole,
                                                          chunk_size=settings.config['general']['chunk_size'],
                                                          temp_dir=settings.config['general']['tempdir'],
                                                          encryption_type=self.blackhole.encryption_type,
                                                          encryption_secret=self.blackhole.encryption_pass,
                                                          delay_between_chunks=settings.config['general'][
                                                              'path_check_interval']):
                            settings.logger_core.debug("Sent `{}` to BlackHole.".format(item.filename))
                    ...
                    try:
                        ...
                        for chunk in item.chunks:
                            if chunk.state == QueueState.UPLOADING:
                                ...
                                settings.TelegramBot.send_chunk_file(chunk=chunk, blackhole=self.blackhole)
                                # Save Queue to disk
                                self.save()
                                self.logger.debug(
                                    f"Rest for {settings.config['general']['path_check_interval']} secs...")
                                time.sleep(settings.config['general']['path_check_interval'])
                        if all_chunks_done:  # If all there is no chunk with UPLOADING state
                            # Add all chunks to Database
                            chunk: WBHChunk
                            for chunk in item.chunks:
                                if chunk.state == QueueState.DONE:
                                    chunk.db_id = settings.Database.add_chunk(chunk=chunk,
                                                                              blackhole_id=self.blackhole.id,
                            ...
                            self.save()
                            # Update chunks_count
                            settings.Database.update_item_chunk_count(item_wbhi=item, chunk_count=len(item.chunks))
                    except Exception as e:
                        settings.logger_core.error(
                            "ERROR: Could add item `{}` to BlackHole: ".format(item.filename, str(e)))
                if item.state == QueueState.DELETED and parent is None:
        ...
        if len(items) == 0 and settings.need_backup:
            settings.need_backup = WBHQueue.backup_database(blackhole=self.blackhole) is None
            settings.logger_core.info("Queue is empty. Backup database is done.")
